...
// ND::TVldRange
//
// ND::TVldRange is delimits/identifies the allowed "context" values
...
ClassImp(ND::TVldRange)

//_____________________________________________________________________________
std::ostream& operator<<(std::ostream& os, const ND::TVldRange& vldr)
{
  return os << vldr.AsString();
}

//_____________________________________________________________________________
ND::TVldRange::TVldRange()
...
//_____________________________________________________________________________
ND::TVldRange::TVldRange(const Int_t detMask, const Int_t simMask,
                         const ND::TVldTimeStamp &tstart,
                         const ND::TVldTimeStamp &tend,
...
//_____________________________________________________________________________
const char* ND::TVldRange::AsString(Option_t *option) const
{
   // Return the ND::TVldRange as a string
   //
   ...
   TString detbits;
   if (opt_a) {
     detbits = ND::DbiDetector::MaskToString(fDetectorMask);
   }
   else {
   ...
   TString simbits;
   if (opt_a) {
     simbits = ND::DbiSimFlag::MaskToString(fSimMask);
   }
   else {
   ...
   // ND::TVldTimeStamp::AsString returns pointer to statically allocated string
...
//_____________________________________________________________________________
Bool_t ND::TVldRange::IsCompatible(const ND::TVldContext &vldc) const
{
   // compare ND::TVldContext with this ND::TVldRange to see if the
   // the tagged set is compatible

   Int_t detector = (Int_t)vldc.GetDetector();
   Int_t simflag  = (Int_t)vldc.GetSimFlag();

   // account for case where both ND::TVldContext and ND::TVldRange
   // are using "kUnknown" which has no bits set
   if ( ! (detector & fDetectorMask) &&
        (detector      != ND::DbiDetector::kUnknown ||
         fDetectorMask != ND::DbiDetector::kUnknown    ) ) return kFALSE;
   if ( ! (simflag  & fSimMask) &&
        (simflag  != ND::DbiSimFlag::kUnknown ||
         fSimMask != ND::DbiSimFlag::kUnknown   ) ) return kFALSE;
...
//_____________________________________________________________________________
void ND::TVldRange::Print(Option_t *option) const
{
   // Print this object
...
//_____________________________________________________________________________
void ND::TVldRange::TrimTo(const ND::TVldRange& vldr)
{
