...
Bool_t CP::TVldRange::IsCompatible(const CP::TVldContext& vldc) const {
    ...
    if (!(detector & fDetectorMask) &&
        (detector      != CP::DbiDetector::kUnknown ||
         fDetectorMask != CP::DbiDetector::kUnknown)) {
        return kFALSE;
    }
    if (!(simflag  & fSimMask) &&
        (simflag  != CP::DbiSimFlag::kUnknown ||
         fSimMask != CP::DbiSimFlag::kUnknown)) {
        return kFALSE;
    }

    // the start time is taken as inclusive, but the end time is exclusive

    if (vldc.GetTimeStamp() <  fTimeStart) {
        return kFALSE;
    }
    if (vldc.GetTimeStamp() >= fTimeEnd) {
        return kFALSE;
    }
...
void CP::TVldRange::TrimTo(const CP::TVldRange& vldr) {
    ...
    if (fTimeStart < vldr.fTimeStart) {
        fTimeStart = vldr.fTimeStart;
    }
    if (fTimeEnd   > vldr.fTimeEnd) {
        fTimeEnd   = vldr.fTimeEnd;
    }
