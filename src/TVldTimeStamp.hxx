...
// The TVldTimeStamp class is essentially a copy of TTimeStamp, so include
// that header to get all of the needed system files.
...
class CP::TVldTimeStamp {
    ...
    friend Bool_t operator==(const CP::TVldTimeStamp& lhs,
                             const CP::TVldTimeStamp& rhs);
    friend Bool_t operator!=(const CP::TVldTimeStamp& lhs,
                             const CP::TVldTimeStamp& rhs);
    friend Bool_t operator< (const CP::TVldTimeStamp& lhs,
                             const CP::TVldTimeStamp& rhs);
    ...
    friend CP::TVldTimeStamp operator- (const CP::TVldTimeStamp& lhs,
                                        const CP::TVldTimeStamp& rhs);
...
public:
    ...
    ///      * -HHMM is the difference between local and UTC (if behind, + if
    ///         ahead).  
    ...
    ///
    /// Equivalent of standard routine "mktime" but using the assumption that
    /// tm struct is filled with UTC, not local, time.
    ///
    /// This version ISN'T configured to handle every possible weirdness of
    /// out-of-range values in the case of normalizing the tm struct.
    ///
    /// This version DOESN'T correctly handle values that can't be fit into a
    /// time_t (i.e. beyond year 2038-01-18 19:14:07, or before the start of
    /// Epoch when time_t is a std::int32_t).
...
namespace CP {
    ...
    inline Bool_t operator!=(const CP::TVldTimeStamp& lhs, 
                             const CP::TVldTimeStamp& rhs) {
        return lhs.fSec  != rhs.fSec ||
               lhs.fNanoSec != rhs.fNanoSec;
    }
    ...
    inline CP::TVldTimeStamp operator-(const CP::TVldTimeStamp& lhs,
                                       const CP::TVldTimeStamp& rhs) {
