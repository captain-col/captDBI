...
ClassImp(ND::TDbiResultSet)

//   Definition of static data members
//   *********************************

 Int_t ND::TDbiResultSet::fgLastID(0);
...
// Writing is a const operation, but uses a non-const method, so cast away const.
  ...
  ND::TDbiResultSet& res_tmp = const_cast< ND::TDbiResultSet&>(res);
  res_tmp.Streamer(bf);
  return bf;
}
...
ND::TDbiResultSet::TDbiResultSet(ND::TDbiInRowStream* resultSet,
...
//
//  Arguments:
//      resultSet     in   Pointer ND::TDbiInRowStream from query. May be null.
...
// None.
  ...
  DbiTrace( "Creating ND::TDbiResultSet" << "  ");
...
ND::TDbiResultSet::~TDbiResultSet() {
//
//
//  Purpose: Destructor
//
...
//  None.
  ...
  DbiTrace( "Destroying ND::TDbiResultSet." << "  ");

  if ( fNumClients ) DbiWarn(  "Warning: Destroying ND::TDbiResultSet with " << fNumClients
    << " clients " << "  ");
...
void ND::TDbiResultSet::BuildLookUpTable() const {
//
//
...
//  Extended Context serach can produce duplicates.
  ...
  for ( Int_t rowNo = this->GetNumRows()-1;
        rowNo >= 0;
        --rowNo ) {
    const ND::TDbiTableRow* row  = this->GetTableRow(rowNo);
...
//  be the right one to use. [Actually ND::TDbiResultSetAgg overrides
//  GetTableRowByIndex, but only to make building lazy].
    const ND::TDbiTableRow* row2 = this->ND::TDbiResultSet::GetTableRowByIndex(index);
...
Bool_t ND::TDbiResultSet::CanDelete(const ND::TDbiResultSet* res) {
//
//
//  Purpose:  See if suitable for deleting.
//
//  Arguments:
//    res          in    A ND::TDbiResultSet that is currently used.
//                       Optional: default = 0.
...
void ND::TDbiResultSet::CaptureExceptionLog(UInt_t startFrom) {
//
//
//  Purpose: Capture Exception Log from latest entries in Global Exception Log.

  fExceptionLog.Clear();
  ND::TDbiExceptionLog::GetGELog().Copy(fExceptionLog,startFrom);

}

//.....................................................................

void ND::TDbiResultSet::GenerateKey() {
//
//
...
const ND::TDbiResultKey* ND::TDbiResultSet::GetKey() const {

//  Purpose:  Get the associated ND::TDbiResultKey, or an empty one if none exists.

  return fKey ? fKey : ND::TDbiResultKey::GetEmptyKey();

}

//.....................................................................

const ND::TDbiTableRow* ND::TDbiResultSet::GetTableRowByIndex(UInt_t index) const {
//
//
...
Bool_t ND::TDbiResultSet::Satisfies(const ND::TVldContext& vc,
...
// Extended Context queries cannot satisfy specific context queries.
  ...
  DbiDebug(  "    Checking result with ND::TDbiValidityRec:- \n      " << this->GetValidityRec()
...
void ND::TDbiResultSet::Streamer(ND::TDbiBinaryFile& file) {
//
//
...
//  Don't store fIndexKeys; it will be rebuilt on input by the subclass.
  ...
  if ( file.IsReading() ) {
    DbiDebug( "    Restoring ND::TDbiResultSet ..." << "  ");
    file >> fCanReuse;
    ...
    DbiDebug( "    Saving ND::TDbiResultSet ..." << "  ");
    file << fCanReuse;
