...
//  Determine the resolution scheme for resolving VLD overlaps.
        ...
        for (UInt_t dbNo = 0; dbNo < numDb && ! foundData; ++dbNo) {
            ...
            if (! proxy.TableExists(dbNo)) {
                continue;
            }
            if (selectDbNo >= 0 && selectDbNo != (int) dbNo) {
                continue;
            }
    ...
    for (; itr != end; ++itr) {
        ...
        if (! vr.IsGap()) {
            if (seqLo == 0) {
                ...
                if (seqLo < vr.GetSeqNo()) {
                    seqLo = vr.GetSeqNo();
                }
                if (seqHi > vr.GetSeqNo()) {
                    seqHi = vr.GetSeqNo();
                }
                if (ts    < vr.GetCreationDate()) {
                    ts    = vr.GetCreationDate();
                }
            }
        }
...
//  Purpose: Return validity record for supplied SeqNo.
    ...
    for (; itr != end; ++itr) if (itr->GetSeqNo() == SeqNo) {
            return *itr;
        }
    return fGap;
}
