...
ClassImp(ND::TDbiResultSetAgg)

typedef vector<const ND::TDbiResultSet*>::const_iterator ConstResultItr_t;
...
///  =============
///
///  o Create Result from ND::TDbiInRowStream generated by query.
///
///
///  Program Notes:-
///  =============
///
///  tableRow is just used to create new subclass ND::TDbiTableRow objects.
///\endverbatim
ND::TDbiResultSetAgg::TDbiResultSetAgg(const string& tableName,
                           const ND::TDbiTableRow* tableRow,
                           ND::TDbiCache* cache,
                           const ND::TDbiValidityRecBuilder* vrecBuilder,
                           const ND::TDbiDBProxy* proxy,
                           const string& sqlQualifiers) :
ND::TDbiResultSet(0,0,sqlQualifiers),
fSize(0)
{
  ...
  DbiTrace( "Creating ND::TDbiResultSetAgg" << "  ");
...
//for Agg No = -1, which should not be present for aggregated data.
  for ( Int_t rowNo = 1; rowNo <= maxRowNo; ++rowNo ) {
    const ND::TDbiValidityRec& vrecRow = vrecBuilder->GetValidityRec(rowNo);

//  If its already in the cache, then just connect it in.
    const ND::TDbiResultSet* res = cache->Search(vrecRow,sqlQualifiers);
...
//  ND::TDbiResultSet and add it to the cache.
    else if ( vrecRow.IsGap() ) {
      ND::TDbiResultSet* newRes = new ND::TDbiResultSetNonAgg(0, tableRow, &vrecRow);
...
//database and build ND::TDbiResultSets for each.
...
//  block ranges of sequence numbers together.
    ...
    ND::TDbiInRowStream* rs = proxy->QuerySeqNos(reqSeqNos,dbNo,sqlData,fillOpts);
//  Flag that data was read from Database.
    this->SetResultsFromDb();
    ND::TDbiTimerManager::gTimerManager.StartSubWatch(1);
    while ( ! rs->IsExhausted() ) {
      ...
      *rs >> seqNo;
      ...
      const ND::TDbiValidityRec& vrecRow = vrecBuilder->GetValidityRec(rowNo);
      ND::TDbiResultSetNonAgg* newRes = new ND::TDbiResultSetNonAgg(rs,tableRow,&vrecRow);
...
//      Adopt but don't register key for this component, only the overall ND::TDbiResultSetAgg
...
//  ND::TDbiInRowStream fully processed, so delete it.
    delete rs;
  }

//All component ND::TDbiResultSetNonAgg objects have now been located and
...
//range by ANDing the time windows together.
  ...
  ND::TDbiValidityRec vRec = vrecBuilder->GetValidityRec(1);
  for ( Int_t rowNo = 1; rowNo <= maxRowNo; ++rowNo ) {

    const ND::TDbiValidityRec& vrecRow = vrecBuilder->GetValidityRec(rowNo);
    ND::TVldRange r = vrecRow.GetVldRange();
    vRec.AndTimeWindow(r.GetTimeStart(),r.GetTimeEnd());

    const ND::TDbiResultSet* res = fResults[rowNo-1];
    if ( res ) {
...
///  =============
///
///  o  Destroy ResultAgg and disconnect all associated ND::TDbiResultSets,
///\endverbatim
ND::TDbiResultSetAgg::~TDbiResultSetAgg() {
...
//  None.
  ...
  DbiTrace( "Destroying ND::TDbiResultSetAgg."  << "  ");
...
///
ND::TDbiResultKey* ND::TDbiResultSetAgg::CreateKey() const {

  ND::TDbiResultKey* key = 0;
  for ( ConstResultItr_t itr = fResults.begin();
        itr != fResults.end();
        ++itr ) {
    const ND::TDbiResultSet* result = *itr;
    if ( result ) {
...
// Should not have an empty set, but just in case.
  if ( ! key ) key = new ND::TDbiResultKey();
...
///\endverbatim
const ND::TDbiTableRow* ND::TDbiResultSetAgg::GetTableRow(UInt_t row) const {
...
///\verbatim
///
///  Purpose:  Get ND::TDbiValidityRec associated with table or row.
///
///  Arguments:
///    row          in    The table row
///                       or null (default) to get table ND::TDbiValidityRec
///  Program Notes:-
///  =============
///
///  The ND::TDbiValidityRec for the in-memory table is effectively an
///  AND of the ND::TDbiValidityRecs of the individual rows.
///
///\endverbatim
const ND::TDbiValidityRec& ND::TDbiResultSetAgg::GetValidityRec(
...
//
 ...
 ND::TDbiResultSet* owner = row->GetOwner();
...
Bool_t ND::TDbiResultSetAgg::Satisfies(const string& sqlQualifiers)  {
//
//
...
///
void ND::TDbiResultSetAgg::Streamer(ND::TDbiBinaryFile& bf) {

//  Specification:-
//  =============

//  Output constituent non-gap ND::TDbiResultSetNonAgg objects.

  vector<const ND::TDbiResultSet*>::const_iterator itr = fResults.begin();
  vector<const ND::TDbiResultSet*>::const_iterator end = fResults.end();

  UInt_t numNonAgg = 0;
  for (; itr != end; ++itr) {
    const ND::TDbiResultSetNonAgg* rna = dynamic_cast<const ND::TDbiResultSetNonAgg*>(*itr);
