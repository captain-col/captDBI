...
public:
    ...
    MsgFormat& fixed()      { fmt=std::ios::fixed;      return *this;}
    MsgFormat& general()    { fmt=std::ios::dec;        return *this;}
    MsgFormat& hex()        { fmt=std::ios::hex;        return *this;}  
    MsgFormat& oct()        { fmt=std::ios::oct;        return *this;}
    MsgFormat& scientific() { fmt=std::ios::scientific; return *this;}
    ...
    MsgFormat& left_justify(int b=1) {
        if (b) { fmt |= std::ios::left; fmt &= (~std::ios::right); }
        else fmt &= ~std::ios::left;
        return *this;
    }
    MsgFormat& right_justify(int b=1) {
        if (b) { fmt |= std::ios::right; fmt &= (~std::ios::left); }
        else fmt &= ~std::ios::right;
        return *this;
    }
    MsgFormat& show_base(int b=1) {
        if (b) fmt |= std::ios::showbase; 
        else fmt &= ~std::ios::showbase; 
        return *this;
    }
    MsgFormat& plus(int b=1) {
        if (b) fmt |= std::ios::showpos; 
        else fmt &= ~std::ios::showpos; 
        return *this;
    }
    MsgFormat& trailing_zeros(int b=1) {
        if (b) fmt |= std::ios::showpoint; 
        else fmt &= ~std::ios::showpoint;
        return *this;
    }
