...
namespace CP {
    ...
    public:
        ...
        TDbiTableMetaData(std::string tableName = "Unknown");
        ...
        Bool_t HasEpoch() const {
            return  this->NumCols() >=4 && this->ColName(4) == "EPOCH";
        }
        ...
        std::string ColName(Int_t colNum) const         {
    ...
    protected:
        ...
        void SetColName(const std::string& name, Int_t colNum)             {
    ...
    private:
        ...
        ///  Return a column attributes (will be dummy entry if requesting
        ///  invalid column)
        const ColumnAttributes& GetAttributes(Int_t colNum) const;

        ///  Return a setable column attributes (will be dummy entry if
        ///  requesting invalid column)
