...
bool CP::TDbiCfgPromptConfigurable::IsInt(const std::string& s, Int_t& i)
{
  ...
  if (endptr==ss && d==0.0) return false; // Conversion to double failed...

  // Check if this number is int or float
  if (strchr(ss,'.')) return false;
  if (strchr(ss,'E')) return false;
  if (strchr(ss,'e')) return false;
				     ...
				     Bool_t recursive )
{
  ...
  while( (key = keyitr()) ) {
   ...
   modify.PrettyPrint(std::cout);
    ...
    if((theType==typeid(char))&&(stuff.Get(key,vchar)))   modify.Set(key,vchar);
    if((theType==typeid(int))&&(stuff.Get(key,vint)))    modify.Set(key,vint);
    if((theType==typeid(double))&&(stuff.Get(key,vdouble))) modify.Set(key,vdouble);
    if((theType==typeid(const char*))&&(stuff.Get(key,vcharstar))) modify.Set(key,vcharstar);
	...
	if(recursive) {
	  ...
	  if(modify.Get(key,old)) {
	    ...
	    old.UnLockValues();
	    old.LockKeys();
	    if(SafeMerge(old,vregistry)) modified = true;
	    modify.Set(key,old);
	  }
