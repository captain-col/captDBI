...
// To run this demo:-
//
// 1) Set up a database (say t2k_db) on a mysql server (say
//    mysqlserver.t2k.com) with a write access account (say
//    t2k_writer,t2k_writer_psw) using the following environmental variables:-
//
...
// 2) Prime it:-
//
//    database_updater.py apply_local_update
//    $OAOFFLINEDATABASEROOT/demo/demo_db_table.update
//
// 3) Run the demo:-
//
...
int main() {
    // Can be boosted up to VerboseLevel / DebugLevel, see CP::TDbiLog
    ...
    for (Int_t current_unix_time = start_unix_time;
        ...
        if (! numRows) {
            ...
            for (Int_t irow = 0; irow<numRows; ++irow) {
                rs.GetRow(irow)->Print();
            }
            const CP::TDemo_DB_Table* required_row 
